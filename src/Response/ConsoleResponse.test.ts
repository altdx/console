...
Deno.test('Altdx Console Response - Should get status code', () => {
  ...
  assertEquals(true, response.setStatus(EResponseType.NOT_FOUND) instanceof ConsoleResponse);
  assertEquals(EResponseType.NOT_FOUND, response.getStatus());
});

Deno.test('Altdx Console Response - Should get status message', () => {
  const response = new ConsoleResponse();
  assertEquals(true, response.setStatusMessage(EResponseType.NOT_FOUND_MESSAGE) instanceof ConsoleResponse);
...
Deno.test('Altdx Console Response - Should check error', () => {
  ...
  assertEquals(true, response.setStatus(EResponseType.SUCCESS) instanceof ConsoleResponse);
